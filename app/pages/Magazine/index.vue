...
import type { RzCategory } from "~~/core/Magazine/articles";
import { pageTitle } from "~~/core/page-title";

/**
 * Magazine home: category chips (filter), search (title/description/tags),
 * responsive card grid, empty/no-result states, and a wide-screen side column
 * with the 5 newest articles (hidden on mobile).
 */
const { t } = useI18n();
const { loading, categories, articles, newest } = useMagazine();
...
const nameOf = (c: RzCategory | undefined) =>
  c
...
const filtered = computed(() => {
  ...
  return articles.value.filter((a) => {
    if (activeCategory.value && a.categoryId !== activeCategory.value)
      return false;
    if (!q) return true;
    const cat = categoryOf(a.categoryId);
    const haystack = [a.title, a.description, ...a.tags, cat ? nameOf(cat) : ""]
      .join(" ")
      .toLowerCase();
...
const hasQuery = computed(
  () => activeCategory.value !== "" || query.value.trim() !== "",
);

/* the SAME title the build-time SEO tags ship (`listHeadHtml`): «مجله — Telepatty» */
useHead(() => ({
  title: pageTitle(t("Magazine.title"), t("app.name")),
  meta: [{ name: "description", content: t("Magazine.tagline") }],
}));
</script>
  ...
  <div class="flex-1 overflow-y-auto overscroll-contain">
    <div class="max-w-6xl w-full mx-auto p-3 flex flex-col gap-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="min-w-0">
          <h1 class="text-lg font-bold">{{ t("Magazine.title") }}</h1>
      ...
      >
        <div
          v-for="i in 3"
          :key="i"
          class="tp-panel aspect-[16/9] animate-pulse"
        />
      </div>
      ...
      <div
        ...
        class="text-center py-16 flex flex-col items-center gap-2"
      >
        <UIcon name="i-lucide-newspaper" class="text-4xl text-dimmed" />
        <p class="text-sm font-medium">{{ t("Magazine.empty") }}</p>
        <p class="text-xs text-dimmed">{{ t("Magazine.emptyHint") }}</p>
      </div>
      <div
      ...
      >
        ...
        <p class="text-sm font-medium">{{ t("Magazine.noResults") }}</p>
      </div>

      <!-- grid + newest sidebar -->
      <div v-else class="flex gap-6 items-start">
        <div
          class="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3"
        >
          <RzArticleCard
            v-for="a in filtered"
            :key="a.slug"
