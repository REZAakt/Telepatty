/**
 * Rooznameh content loader.
 *
 * Content lives in `content/rooznameh/*.md` at the project root (categories in
 ...
 * this composable.
 *
 * Parsing/validation happens in `core/rooznameh/*` (pure + unit-tested); this
...
import {
  ...
  type RzArticle,
  type RzCategory,
} from '~~/core/rooznameh/articles'
import { splitFrontmatter } from '~~/core/rooznameh/frontmatter'

/** build-time glob: relative to this file → <project>/content/rooznameh/*.md */
const modules = import.meta.glob('../../content/rooznameh/*.md', {
  query: '?raw',
  import: 'default',
}) as Record<string, () => Promise<string>>

export interface RooznamehData {
  categories: RzCategory[]
  /** sorted newest-first, drafts excluded */
  articles: RzArticle[]
}

let cache: RooznamehData | null = null
let pending: Promise<RooznamehData> | null = null
...
async function load(baseURL: string): Promise<RooznamehData> {
  // categories file first (underscore = not an article)
  const rawEntries = await Promise.all(
    Object.entries(modules).map(async ([path, loadRaw]) => [path, await loadRaw()] as const),
  )
  const categoriesSource = rawEntries.find(([path]) => slugOf(path) === '_categories')
  const categories = normalizeCategories(splitFrontmatter(categoriesSource?.[1] ?? '').data)
...
export function useRooznameh() {
  const data = ref<RooznamehData | null>(cache)
  ...
  const findArticle = (slug: string): RzArticle | undefined => articles.value.find((a) => a.slug === slug)
  ...
  const related = (slug: string, categoryId: string | null): RzArticle[] => {
    const same = articles.value.filter((a) => a.slug !== slug && a.categoryId && a.categoryId === categoryId)
    return same.slice(0, 3)
  }

  return { loading: readonly(loading), categories, articles, newest, findArticle, neighbours, related }
}