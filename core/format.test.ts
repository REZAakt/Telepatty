...
describe("formatting", () => {
  ...
  it("formats message time without crashing in both locales/calendars", () => {
    ...
    const en = formatMessageTime(ts, {
      locale: "en",
      jalali: false,
      persianDigits: false,
    });
    const faJalali = formatMessageTime(ts, {
      locale: "fa",
      jalali: true,
      persianDigits: true,
    });
   ...
   * the calendar setting a no-op in Persian — chat and Magazine looked the same
  ...
  it("honors the calendar setting in both languages", () => {
    const ts = new Date(2024, 0, 15, 10, 30).getTime();
    const opts = (locale: "fa" | "en", jalali: boolean) => ({
      locale,
      jalali,
      persianDigits: false,
    });
  ...
  it("honors the digit setting in both languages", () => {
    ...
    const latin = formatDateSeparator(ts, {
      locale: "fa",
      jalali: true,
      persianDigits: false,
    });
    ...
    const persian = formatDateSeparator(ts, {
      locale: "fa",
      jalali: true,
      persianDigits: true,
    });
