...
function precacheGlobs(): string[] {
  return [...arrayLiteral('globPatterns').matchAll(/'([^']+)'/g)].map((m) => m[1])
}

/** The `navigateFallbackDenylist` regex literals, compiled exactly as workbox does. */
function navigationDenyList(): RegExp[] {
  return [...arrayLiteral('navigateFallbackDenylist').matchAll(/\/((?:\\.|[^/\\])+)\/([a-z]*)/g)]
...
function globToRegExp(glob: string): RegExp {
  ...
  if (brace) {
    patterns.splice(0, 1, ...brace[1].split(',').map((alt) => glob.replace(brace[0], alt.trim())))
  }
...
describe('crawler files vs the PWA navigation fallback', () => {
  it('precaches sitemap.xml and robots.txt (xml/txt are in the workbox glob)', () => {
    ...
    const cached = (file: string) => globs.some((g) => globToRegExp(g).test(file))
    ...
    for (const file of ['index.html', '_nuxt/entry.js', 'favicon.ico', 'icons/icon-192.png']) {
      expect(cached(file), file).toBe(true)
    }
  })

  it('never answers a browser navigation to a crawler file with the SPA shell', () => {
    const denied = (path: string) => navigationDenyList().some((re) => re.test(path))
    ...
    for (const route of ['/', '/friends', '/rooznameh', '/rooznameh/add-friends', '/settings']) {
...
describe('@nuxtjs/sitemap wiring (the sitemap is generated, never authored)', () => {
  it('registers the module and pins lastmod to the content, not the build clock', () => {
    ...
    expect(/sitemap:\s*\{[\s\S]*?urls:\s*\(\)\s*=>\s*sitemapUrls\(/.test(raw)).toBe(true)
...
describe('the internal app screens stay out of the sitemap', () => {
  ...
  const excludePatterns = () =>
    [...arrayLiteral('exclude').matchAll(/'([^']+)'/g)].map((m) => globToRegExp(m[1]!))
  const excluded = (path: string) => excludePatterns().some((re) => re.test(path))

  it('excludes /add, /lock and /onboarding (and any future subroute under them)', () => {
    for (const path of ['/add', '/lock', '/onboarding', '/add/invite', '/lock/verify', '/onboarding/step-2']) {
      expect(excluded(path), path).toBe(true)
    }
  })

  it('touches no indexable page and no article', () => {
    const kept = ['/', '/friends', '/settings', '/rooznameh', '/rooznameh/add-friends', '/rooznameh/hello-rooznameh']
