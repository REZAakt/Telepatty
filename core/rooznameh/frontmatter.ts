/**
 * Rooznameh (روزنامه) frontmatter — a pragmatic YAML-subset parser.
 *
...
function parseBlockList(lines: string[], start: number, seek: (next: number) => void): unknown[] {
  const items: unknown[] = []
  let i = start
  while (i < lines.length) {
    ...
    const item: Record<string, unknown> = { [kv[1]!]: coerceScalar(kv[2] ?? '') }
    while (i < lines.length) {
