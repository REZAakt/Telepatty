...
import { articleRoutes, basePath, listArticles, normaliseLastmod, sitemapUrls } from './sitemap'

const ORIGIN = 'https://telepatty.ir'
/** The REAL content folder — the source of truth the build reads. */
const CONTENT_DIR = join(process.cwd(), 'content/rooznameh')

/** Temp markdown files this suite drops into `content/` (always removed again). */
const tempFiles: string[] = []
function addTempArticle(name: string, frontmatter: string, body = 'Body text.'): string {
...
const entryFor = (slug: string) => urls().find((u) => u.loc.endsWith(`/rooznameh/${slug}`))
...
describe('a new markdown file is all it takes to enter the sitemap', () => {
  ...
  it('adds the article URL on the next build, with ITS frontmatter date — and drops it when the file is gone', () => {
    expect(locs()).not.toContain(`${ORIGIN}/rooznameh/tmp-sitemap-article`)

    const file = addTempArticle(NAME, "title: 'Temp sitemap article'\ndate: 2026-01-02")
    expect(locs()).toContain(`${ORIGIN}/rooznameh/tmp-sitemap-article`)
    ...
    expect(articleRoutes(listArticles(CONTENT_DIR))).toContain('/rooznameh/tmp-sitemap-article')
  ...
  it('matches the published markdown exactly (the count the build logs)', () => {
    ...
    const expected = files.filter((f) => {
      const { data } = splitFrontmatter(readFileSync(join(CONTENT_DIR, f), 'utf8'))
      return data.draft !== true && data.draft !== 'true' && String(data.title ?? '').trim() !== ''
    })
    expect(urls().length).toBe(expected.length)
    // every entry is absolute, unique and lives under the Rooznameh path
    const entries = urls()
    expect(new Set(entries.map((e) => e.loc)).size).toBe(entries.length)
    expect(entries.every((e) => e.loc.startsWith(`${ORIGIN}/rooznameh/`))).toBe(true)
  })
})
...
describe('excluded files never reach the sitemap (same rules as the app)', () => {
  it('skips drafts, title-less files and `_`-prefixed files', () => {
    addTempArticle('tmp-draft.md', "title: 'Draft'\ndate: 2026-01-03\ndraft: true")
    addTempArticle('tmp-draft-string.md', "title: 'Draft'\ndate: 2026-01-03\ndraft: 'true'")
    addTempArticle('tmp-notitle.md', 'date: 2026-01-04')
    addTempArticle('_tmp-partial.md', "title: 'Looks like an article'\ndate: 2026-01-05")
    ...
    expect(locs()).toContain(`${ORIGIN}/rooznameh/hello-rooznameh`)
  })
})
...
describe('dates come from the content, never from the build clock', () => {
  ...
  it('an article without a date gets NO lastmod at all (never "now")', () => {
    ...
    expect(entryFor('tmp-nodate')).toEqual({ loc: `${ORIGIN}/rooznameh/tmp-nodate` })
    expect('lastmod' in entryFor('tmp-nodate')!).toBe(false)

    addTempArticle('tmp-updated.md', "title: 'Updated'\ndate: 2026-01-02\nupdated: 2026-03-04")
...
describe('url shapes', () => {
  it('prefixes the origin + baseURL and never doubles slashes', () => {
    ...
    expect(basePath('preview')).toBe('/preview')
    expect(
      sitemapUrls([{ slug: 'a', lastmod: '' }], { origin: 'https://telepatty.ir/', baseURL: '/preview/' }),
    ).toEqual([{ loc: 'https://telepatty.ir/preview/rooznameh/a' }])
  })

  it('articleRoutes() mirrors the URL slugs as app routes', () => {
    expect(articleRoutes([{ slug: 'x', lastmod: '2026-01-01' }, { slug: 'y', lastmod: '' }])).toEqual([
      '/rooznameh/x',
      '/rooznameh/y',
    ])
  })
