...
describe("appearanceToCss", () => {
  ...
  it("emits light surfaces when resolved to light (white theme must restyle the page)", () => {
    const css = appearanceToCss(
      { ...DEFAULT_APPEARANCE, colorMode: "light" },
      "light",
    );
   ...
   * another primary). This is the fix for “Settings + Magazine stay green”.
   */
  it("follows the live Nuxt UI primary token when no explicit accent is set", () => {
    expect(appearanceToCss({ ...DEFAULT_APPEARANCE })["--tp-accent"]).toBe(
      "var(--ui-primary)",
    );
