 ...
 *
 * Magazine shipped with "placeholder screenshot" / "replace with
 ...
 */
const CONTENT = join(process.cwd(), "content/Magazine");
const MEDIA = join(process.cwd(), "public/media");

const categories = normalizeCategories(
  splitFrontmatter(readFileSync(join(CONTENT, "_categories.md"), "utf8")).data,
);
...
function referencedFiles(slug: string): string[] {
  const { article } = buildArticle(
    slug,
    readFileSync(join(CONTENT, `${slug}.md`), "utf8"),
    categories,
  );
  if (!article) return [];
  const names: string[] = [];
...
describe("magazine assets are real, present and publishable", () => {
  ...
  it("every image/video an article references exists under public/media", () => {
    for (const slug of slugs) {
      ...
      expect(refs.length, `${slug} references no media at all`).toBeGreaterThan(
        0,
      );
      for (const name of refs) {
        ...
        expect(
          existsSync(join(MEDIA, clean!)),
          `${slug}: public/media/${clean} is missing`,
        ).toBe(true);
      }
    }
  });
  ...
  it("no SVG under public/media is a leftover placeholder or authoring note", () => {
    const svgs = readdirSync(MEDIA).filter((f) =>
    ...
    for (const file of svgs) {
      ...
      expect(
        ...
        `public/media/${file} still says "placeholder"`,
      ).toBe(false);
      expect(
        text.includes("replace with /media"),
        `public/media/${file} still carries an authoring note`,
      ).toBe(false);
    }
  });
  ...
  it("every SVG in public/media is well-formed and scalable", () => {
    ...
    for (const file of svgs) {
      const doc = parser.parseFromString(
        readFileSync(join(MEDIA, file), "utf8"),
        "image/svg+xml",
      );
      const error = doc.querySelector("parsererror");
      expect(
        error?.textContent ?? null,
        `public/media/${file} is not valid XML`,
      ).toBeNull();
      expect(
      ...
      expect(
        ...
        `public/media/${file} has no viewBox`,
      ).toBeTruthy();
    }
  });
});
