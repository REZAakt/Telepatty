/**
 * Magazine block-directive parser (pure, unit-testable).
 *
...
export function parseDirectives(body: string): RzBlock[] {
  ...
  const push = (b: RzBlock | null): void => {
    ...
    while (b.lines.length && !b.lines[b.lines.length - 1]!.trim())
      b.lines.pop();
...
export function parseAdBlock(lines: string[]): AdContent {
  ...
  const hasImage =
    first && !/^(https?:)?\/\//i.test(first) && !first.startsWith("/");
...
export function parseQuoteLines(lines: string[]): {
  md: string;
  source?: string;
} {
...
export function parseCalloutLines(lines: string[]): {
  variant: "info" | "warning";
  md: string;
} {
...
export function parseSourceLine(line: string): SourceContent {
  ...
  return {
    text,
    url: caption && /^https?:\/\//i.test(caption) ? caption : undefined,
  };
}
