/**
 * Magazine article & category model (pure, unit-testable).
 * The composable (`app/composables/useMagazine.ts`) feeds raw .md strings in;
...
export function normalizeCategories(
  ...
  for (const item of rawList) {
    ...
    out.push({
      ...
      nameFa: String(
        rec.name_fa ?? rec.nameFa ?? rec.name_en ?? rec.nameEn ?? id,
      ),
      color:
...
export function validateCategory(
  ...
  return {
    id: null,
    warning: `[Magazine] category "${sid}" is not defined in content/Magazine/_categories.md — add it or fix the article's frontmatter.`,
  };
}
...
export function buildArticle(
  ...
  if (!title) {
    warnings.push(`[Magazine] ${slug}.md has no title — article skipped.`);
    return { article: null, warnings };
  }
  ...
  if (dateText && ISO_DATE.test(dateText)) {
    ...
    if (Number.isFinite(parsed)) date = parsed;
      ...
      warnings.push(
        `[Magazine] ${slug}.md has an unparsable date "${dateText}" — using now.`,
      );
  } else if (dateText) {
    warnings.push(
      `[Magazine] ${slug}.md date should be YYYY-MM-DD (got "${dateText}") — using now.`,
    );
  }

  const cat = validateCategory(data.category, categories);
  if (cat.warning) warnings.push(`[Magazine] ${slug}.md: ${cat.warning}`);
  ...
  if (data.cover) {
    ...
    if (!cover)
      warnings.push(
        `[Magazine] ${slug}.md cover "${asString(data.cover)}" is not a safe /media path — ignored.`,
      );
  }

  const blocks = parseDirectives(fm.body);
  const unknownDirectives = [
    ...new Set(blocks.map((b) => b.name).filter((n) => !isKnownDirective(n))),
  ];
  if (unknownDirectives.length) {
    warnings.push(
      `[Magazine] ${slug}.md uses unknown directive(s) ${unknownDirectives.map((n) => `#${n}`).join(", ")} — rendered as plain text.`,
    );
  }
  ...
  return {
    article: {
      ...
      readingMinutes: readingMinutes(
        `${title} ${asString(data.description)} ${fm.body}`,
      ),
      blocks,
      unknownDirectives,
    },
