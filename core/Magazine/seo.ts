/**
 * Build-time SEO for Magazine (pure, unit-testable — no fs, no DOM).
 *
 * Why this exists: the app is `ssr: false`, so the prerendered
 * `/Magazine/<slug>/index.html` used to be a bare app shell — social scrapers
 ...
 *
 * At generate time (`scripts/inject-Magazine-head.ts`) each prerendered shell
...
export function resolveOgImage(
  ...
  if (!OG_EXTENSIONS.test(article.cover)) {
    return {
      ...fallback,
      warning: `[Magazine] ${article.slug}: og:image needs PNG/JPG — cover ${article.cover} is not usable, using media/og-default.png (add a PNG/JPG cover, ideally 1200×630).`,
    };
  }
  if (opts.exists && !opts.exists(article.cover)) {
    return {
      ...fallback,
      warning: `[Magazine] ${article.slug}: cover ${article.cover} not found in the build output — og:image falls back to media/og-default.png.`,
    };
  }
...
function publishedTime(article: Pick<RzArticle, "date" | "dateText">): string {
  if (DATE_ONLY_RE.test(article.dateText))
    return `${article.dateText}T00:00:00Z`;
  if (FULL_ISO_RE.test(article.dateText))
    return article.dateText.replace(" ", "T");
  return new Date(article.date).toISOString();
}

function canonicalFor(
  article: Pick<RzArticle, "slug">,
  opts: SeoOriginOptions,
): string {
  return `${opts.origin}${opts.baseURL}Magazine/${article.slug}/`;
}
...
export function articleHeadHtml(
  ...
  return [
    ...
    meta(
      `property="article:published_time" content="${escapeHtml(published)}"`,
    ),
...
/** Generic <head> tags for the /Magazine/ index page. */
export function listHeadHtml(opts: SeoOriginOptions): string {
  const canonical = `${opts.origin}${opts.baseURL}Magazine/`;
  const title = "Magazine — Telepatty";
  const description =
    "The Telepatty magazine: articles and announcements from the serverless messenger.";
...
function textBlockHtml(lines: string[], out: string[]): void {
  ...
  const flush = () => {
    ...
    out.push(
      `  <p>${escapeHtml(markdownToPlainText(para.join("\n")).trim())}</p>`,
    );
    para = [];
  };
...
export function articleBodyHtml(article: RzArticle): string {
  ...
  for (const block of article.blocks) {
    if (
      block.name === "text" ||
      !["media", "ad", "quote", "callout", "source"].includes(block.name)
    ) {
      textBlockHtml(block.lines, out);
    } else if (block.name === "quote") {
      out.push(
        "  <blockquote>" +
          escapeHtml(markdownToPlainText(block.lines.join(" ")).trim()) +
          "</blockquote>",
      );
    } else if (block.name === "callout") {
      const [first = "", ...rest] = block.lines;
      const body = (
        /^(info|warning)$/i.test(first.trim()) ? rest : block.lines
      ).join(" ");
      out.push(
        "  <p><strong>" +
          escapeHtml(markdownToPlainText(body).trim()) +
          "</strong></p>",
      );
