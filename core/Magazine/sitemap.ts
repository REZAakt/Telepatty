/**
 * Magazine (مجله) → sitemap source (fs + pure helpers, unit-tested).
 *
 ...
 * (`splitFrontmatter`). Dropping a markdown file into `content/Magazine/` is
...
/** `/Magazine/<slug>` per article — the routes Nitro must prerender (200, not 404). */
export function articleRoutes(articles: SitemapArticle[]): string[] {
  return articles.map((a) => `/Magazine/${a.slug}`);
}
