...
function article(fm: string, body: string) {
  const categories = normalizeCategories({
    categories: [{ id: "tech", name_en: "Technology", name_fa: "تکنولوژی" }],
  });
...
describe("markdownToPlainText", () => {
  it("keeps link text, drops images to alt, strips emphasis", () => {
    expect(markdownToPlainText("[click **here**](https://x.ex)")).toBe(
      "click here",
    );
...
describe("resolveOgImage (og:image must be an absolute PNG/JPG URL)", () => {
  it("accepts an existing PNG/JPG cover and makes it absolute", () => {
    const r = resolveOgImage(
      { slug: "a", cover: "/media/cover.png" },
      { origin: ORIGIN, baseURL: BASE, exists: () => true },
    );
    expect(r).toEqual({
      url: "https://telepatty.ir/media/cover.png",
      fallback: false,
    });
  });
  it("falls back with a warning for SVG covers", () => {
    const r = resolveOgImage(
      { slug: "a", cover: "/media/cover.svg" },
      { origin: ORIGIN, baseURL: BASE, exists: () => true },
    );
  ...
  it("falls back with a warning when the cover file is missing", () => {
    const r = resolveOgImage(
      { slug: "a", cover: "/media/cover.jpg" },
      { origin: ORIGIN, baseURL: BASE, exists: () => false },
    );
    expect(r.fallback).toBe(true);
  ...
  it("falls back silently when there is no cover at all", () => {
    ...
    expect(r).toEqual({
      url: "https://telepatty.ir/media/og-default.png",
      fallback: true,
    });
  });
  it("honors a baseURL sub-path (GitHub Pages project sites)", () => {
    ...
    const r = resolveOgImage(
      { slug: "a", cover: "/tp/media/cover.jpg" },
      { origin: "https://x.ex", baseURL: "/tp/", exists: () => true },
    );
...
describe("articleHeadHtml (head-injection logic)", () => {
  ...
  it("emits every required tag", () => {
    ...
    expect(html).toContain(
      'rel="canonical" href="https://telepatty.ir/Magazine/hello-world/"',
    );
    expect(html).toContain('property="og:title"');
    expect(html).toContain('property="og:description"');
    expect(html).toContain('property="og:type" content="article"');
    expect(html).toContain(
      'property="og:url" content="https://telepatty.ir/Magazine/hello-world/"',
    );
    expect(html).toContain(
      'property="og:image" content="https://telepatty.ir/media/cover.png"',
    );
    expect(html).toContain('name="twitter:card" content="summary_large_image"');
    // plain YYYY-MM-DD frontmatter dates are published at midnight UTC (stable
    // across build machines / timezones)
    expect(html).toContain(
      'property="article:published_time" content="2026-09-21T00:00:00Z"',
    );
  });
...
describe("listHeadHtml (the /Magazine/ index page)", () => {
  const html = listHeadHtml({ origin: ORIGIN, baseURL: BASE });
  it("emits generic title/description/og/twitter tags", () => {
    expect(html).toContain("<title>Magazine — Telepatty</title>");
    expect(html).toContain(
      'rel="canonical" href="https://telepatty.ir/Magazine/"',
    );
...
<script type="module" src="/_nuxt/entry.js"></script>
  ...
  it("replaces generic head tags and injects article head + crawlable body", () => {
    const out = injectIntoShell(
      SHELL,
      '    <title>Article</title>\n    <meta property="og:title" content="Article">',
      '<div id="tp-rz-seo"><h1>Article</h1></div>',
    );
    ...
    expect(out.indexOf('id="tp-rz-seo"')).toBeGreaterThan(
      out.indexOf('id="__nuxt"'),
    );
    expect(out).toContain('</div>\n<script type="module"');
  });

  it("is idempotent (running twice does not duplicate tags)", () => {
    const once = injectIntoShell(
      SHELL,
      "    <title>A</title>",
      '<div id="tp-rz-seo"><h1>A</h1></div>',
    );
    const twice = injectIntoShell(
      once,
      "    <title>A</title>",
      '<div id="tp-rz-seo"><h1>A</h1></div>',
    );
