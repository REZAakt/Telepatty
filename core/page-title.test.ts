...
describe('pageTitle (browser-tab titles)', () => {
  ...
  it('does not repeat the app name when the screen string already carries it', () => {
    expect(pageTitle('Telepatty قفل است', 'Telepatty')).toBe('Telepatty قفل است')
    expect(pageTitle('Welcome to Telepatty', 'Telepatty')).toBe('Welcome to Telepatty')
  })
})
 ...
 * Before this, only the two Rooznameh pages set a head and every other tab just
...
describe('every page titles the browser tab (grep guard)', () => {
  ...
  it('the Rooznameh list title matches its own build-time SEO title', () => {
    const src = readFileSync(join(process.cwd(), 'app/pages/rooznameh/index.vue'), 'utf8')
